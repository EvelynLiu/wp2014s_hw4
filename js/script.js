...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
	...
	ctx.fillText("Draw a line here to start editing your photo", 40, 270); //設定預設的開始畫面
    var img = new Image(); // 新增圖像1
    img.src = "img/PinkFrame.jpg"; //圖像路徑（路徑自己設，且自己加入想要的圖層）
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
			...
			ctx.font='30px "微軟正黑體"'; //字體大小和字形
			ctx.fillText(inputedText,230,450); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log("success " + data);//成功log + photoID
                  $(".info").html("Posted Canvas Successfully. [<a href='http://www.facebook.com/" + data.id + ?preview_cover" '>Go to Profile Picture</a>] "); //成功訊息並顯示連接
            },
